...
var Extractor = (function () {
    function Extractor(options) {
        this.options = _.extend({
            ...
            lineNumbers: true,
            stringLength: false,
            stringLengthFactor: 1.3,
            fileRefNormalized: false,
            extensions: {
                htm: 'html',
                html: 'html',
                php: 'html',
    ...
    Extractor.prototype.addString = function (reference, string, plural, extractedComment, context) {
        ...
        if (!this.strings[string][context]) {
            ...
            if (this.options.stringLength) {
                this.strings[string][context].extractedComments.push('String Length: ' +
                    Math.round(string.length * this.options.stringLengthFactor + 0.5))
            }
        }
        ...
        if (this.options.fileRefNormalized) {

            if (!~['posix', 'win32'].indexOf(this.options.fileRefNormalized)) {
                this.options.fileRefNormalized = 'posix';
            }

            refString = path[this.options.fileRefNormalized].join(refString);
        }
